...
$(function () {
    ...
    // List Page
    var _areaAction = $('.sort-multiple');
    var _selectSortList = $('.sort-multiple .select-sort-list');

    _areaAction.find('.sort-by').hide();

    _selectSortList.find('select').change(function () {
        var _valSortType = _selectSortList.find('.select option:selected').val();
        if (_valSortType != "") {
            _areaAction.find('.sort-by').hide();
            _areaAction.find('#' + _valSortType + '.sort-by').show();
        } else {
            _areaAction.find('.sort-by').hide();
        }

        _areaAction.find('.multiselect-select-all option:selected').each(function () {
            $(this).prop('selected', false);
        })
        _areaAction.find('.multiselect-select-all').multiselect('refresh');
        $.uniform.update();
    });
    ...
    var _listTable = $('table.tasks-list');
    var _listReorder = '#sortable-table';
    var _rowCount = _listTable.find('> tbody  > tr').length;

    _listTable.not(_listReorder).find('tbody tr').slice(-3).find('.dropdown, .btn-group').addClass('dropup');

    if (_rowCount < 5 && _bodyWidth <= 768) {
        $('.table-responsive').find('.dropdown-menu, .dropdown-toggle').css('position', 'static');
    }
    ...
    // ========= Sortable ==========
    $(".show-list-all").find(".btn-reorder").click(function () {
        $(".panel-body").find(".show-list-all").hide();
        $(".panel-body").find(".show-reorder-all").show();
    });
